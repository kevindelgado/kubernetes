...
var onListError = func(error) bool { return true }
...
func TestGarbageCollectorConstruction(t *testing.T) {
	...
	sharedInformers := informers.NewSharedInformerFactoryWithOptions(client, 0, informers.WithOnListError(onListError))
...
func TestCRDDeletionCascading(t *testing.T) {
	// FIRST SYNC (foo not present, pod is)
	// GetDeletableResources doesn't have foo crd
	// reset restMapper
	// resyncMonitors
	// wait for cache sync

	// add crd to discovery
	// create pod with owner ref
	// make add event(pod with foo owner)
	// processAttemptToDelete virtual?
	// create foo instance with pod dep
	// make add event(foo owner)
	// processAttemptToDelete virtual?
	// crd and pod successfully created
	//
	// SECOND SYNC (foo crd and pod present)
	// GetDeletableResources doesn't have foo crd
	// reset restMapper
	// resyncMonitors
	// wait for cache sync

	// REMOVE crd from discovery
	// GC RUN
	// GetDeletableResources doesn't have foo crd
	// reset restMapper
	// resyncMonitors
...
//func TestCRDDeletionCascading(t *testing.T) {
//	t.Logf("starting crd test")
//	// setup the discovery client
//	serverResources := []*metav1.APIResourceList{
//		{
//			GroupVersion: "v1",
//			APIResources: []metav1.APIResource{
...
//		InterfaceUsedCount: 0,
//	}
//	serverResourcesPostInstall := []*metav1.APIResourceList{
//		{
//			GroupVersion: "v1",
//			APIResources: []metav1.APIResource{
//				{Name: "pods", Namespaced: true, Kind: "Pod", Verbs: metav1.Verbs{"delete", "list", "watch"}},
...
//			APIResources: []metav1.APIResource{
//				{Name: "foos", Namespaced: true, Kind: "Foo", Verbs: metav1.Verbs{"delete", "list", "watch"}},
//			},
//		},
//	}
//
...
//	testHandler := &fakeActionHandler{
//		response: map[string]FakeResponse{
//			"GET" + "/api/v1/namespaces/ns1/pods": {
//				200,
//				[]byte("{}"),
//			},
//		},
//	}
//
//	//testHandlerPostInstall := &fakeActionHandler{
//	//	response: map[string]FakeResponse{
//	//		"GET" + "/api/v1/namespaces/ns1/pods": {
//	//			200,
//	//			[]byte("{}"),
//	//		},
//	//		"GET" + "/api/v1/namespaces/ns1/foos": {
//	//			200,
//	//			[]byte("{}"),
//	//		},
//	//	},
//	//}
//	// obtain config and start tets server
//	srv, clientConfig := testServerAndClientConfig(testHandler.ServeHTTP)
//	defer srv.Close()
//
//	// setup
...
//	// defer close(gc.stop)
//	// NEW:
//	// there's a difference between stupGC and the setup TestSync uses
//	// based on Sync using a client from the config rather than a fake.NewSimpleClientSet
//	clientConfig.ContentConfig.NegotiatedSerializer = nil
//	client, err := kubernetes.NewForConfig(clientConfig)
//	if err != nil {
//		t.Fatal(err)
...
//	go gc.Run(1, stopCh)
//	// first sync
//	go gc.Sync(fakeDiscoveryClient, 1*time.Second, stopCh)
//	time.Sleep(3 * time.Second)
//	close(stopCh)
//
//	// install crd
//	// add foo to discovery doc
//	fakeDiscoveryClient.PreferredResources = serverResourcesPostInstall
//	// add endpoint for foos
//	//
//
//	// create cr
//	// uninstall crd
//	// second sync
//	//
//	// TODO: do this before trying to do the install/uninstall second sync stuff
//	// confirm attempt to delete
//}
