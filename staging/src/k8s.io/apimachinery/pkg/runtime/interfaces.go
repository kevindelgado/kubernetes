...
// kdelga: To begin, the serilizer interface (and the corresponding Encoder/Decoder) interfaces are what transform
// k8s objects into their serialized form to be written across the wire. It's worth mentioning that kubernetes supports multiple
// serialization formats, default is JSON, but also protobuf and these Encoder/Decoder interfaces abstract away the differences
...
// kdelga: Codec, while provideing the same interface as a Serializer cares about the versioning of objects.
// So while a serializer converts objects to and from wire formats, a Codec can additionally modify the data
// in order to produce results compatible with the target version.

// Similarly, for defaulting, the goal is for older clients to continue to work even when they don't understand new fields.
// The server is than able to use a Codec to add default values to these new fields.
...
// kdelga: This NegotiatedSerialize is what we use to get a serializer for the "media type" we
// are encoding to/ decoding from be it json or protobuf.
// And here with the ForVersion ToVersion is where conversion comes in that lets older clients speak to newer api servers
// by converting k8s objects from the old clients into a form understandable by the newer api servers
...
// kdelga: Quick aside section 2:
// now that we've talked about what happens on the client when you make a kubectl call, before diving into how the apiserver handles the request
// it makes sense to discuss some of the autogenerated machinery that straddles both the client and server
// and is critical for helping them understand each other.
// (Factcheck: does conversion and defaulting actually straddle or only live on the)
//
// This auto-generated machinery primarily consists of
// * Serialization - representing structures in memory as bytes on the wire
// * Conversion - letting newer API versions understand objects from clients written against old API versions
// * Defaulting - understanding clients that are unaware of new API fields.

// At its core, the (previously mentioned runtime.Encode)
// To begin to understand this auto-generated machinery we starte with the runtime.Object interface.
// This is what go structs that want to be used as kubernetes API objects must conform to inorder to
// be understood by both client and server.
// Historically this was 
// blah blah, not sure if I like this bit.
